...
##    def _test_encryption_decryption(self, cipher_class, message, encrypt_args, decrypt_args, expected_encrypted, expected_decrypted=None):
##        cipher = cipher_class()
##        encrypted = cipher.encrypt(message, *encrypt_args)
##        self.assertEqual(encrypted, expected_encrypted)
##
##        if expected_decrypted is None:
##            expected_decrypted = message
##
##        decrypted = cipher.decrypt(encrypted, *decrypt_args)
...
##    ])
##    def test_caesarCipher(self, name, message, key, expected):
##        TestCipher(CaesarCipher(), "standard", "Hello", 3, "Khoor").test()
##        self.test_encryption_decryption(CaesarCipher, name, message, key, expected)
##
##
##    # ROT13 CIPHER TESTS
##    @parameterized.expand([
